...
fviz_pca_ind(pca, habillage=rawmetadata$tortoise_group, geom="point", addEllipses=F, pointshape=19, point.size=1, mean.point=T, repel=T) + labs(title="PhILR Distance") + scale_color_brewer(palette="Dark2") + theme_minimal() + xlim(-110,160) + ylim(-110,160)
dev.off()
...
plot_richness(ps.dat, x="tortoise_group", measures=c("Observed", "Shannon")) + theme_minimal()
dev.off()
pdf("figs/adiv_age.pdf")
plot_richness(ps.dat, x="agegroup", color="tortoise_group", measures=c("Observed", "Shannon")) + theme_minimal()
dev.off()

###############################
#Microbial co-occurence network
###############################
d <- ncol(seqtab.filtered)
n <- nrow(seqtab.filtered)
e <- d
set.seed(10010)
